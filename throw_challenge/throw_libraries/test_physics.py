        ...
        for level_num, config in LEVELS.items():
            ...
            if level_num == 5:
                ...
                for pos in range(-20, int(target_dist) + 5):
                    landing_x = physics.calculate_landing_x(velocity, angle, start_x=pos, start_height=start_height, target_height=target_height, target_dist=target_dist, target_width=target_width)
                ...
                for velocity in range(10, 121):
                    ...
                    for angle in range(10, 90):
                        landing_x = physics.calculate_landing_x(velocity, angle, start_x=0, start_height=start_height, target_height=target_height, target_dist=target_dist, target_width=target_width)
                        if abs(landing_x - target_dist) <= tolerance:
                            solution_found = True
                            break
