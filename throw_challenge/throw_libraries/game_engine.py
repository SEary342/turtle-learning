...
def setup_level(level_number) -> bool:
    ...
    view_height = max(
        current_level_config["start_height"] + 30,
        current_level_config.get("target_height", 0) + 30,
        40,
    )
    ...
    visuals.draw_target(
        current_level_config["target_dist"],
        current_level_config["target_width"],
        current_level_config.get("target_height", 0),
    )
...
def throw_ball(velocity, angle, robot_x=0):
    ...
    if velocity > MAX_VELOCITY:
        ...
        print(
            f"Error: {velocity} m/s is too fast! Please keep it under {MAX_VELOCITY} m/s."
        )
        return
    ...
    # Calculate result
    flight_path = physics.simulate_flight(
        velocity,
        angle,
        start_x=robot_x,
        start_height=start_h,
